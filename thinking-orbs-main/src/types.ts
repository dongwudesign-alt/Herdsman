...
import type { Rgb } from './engine/core';
...
export interface ThinkingOrbProps extends Omit<CanvasHTMLAttributes<HTMLCanvasElement>, 'style'> {
  ...
  /**
   * RGB ink tint for the dots, e.g. `[96, 165, 250]` for blue. Ink level
   * scales the tint so depth shading is preserved; `null` keeps the
   * original grayscale ink. @default null
   */
  color?: Rgb | null;
