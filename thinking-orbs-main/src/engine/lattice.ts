...
export const drawGlobe: ModeDraw = (ctx, size, t, dark, o, rgb) => {
  const spin = 0.5;
  const cx = size / 2;
  const cy = size / 2;
  ...
  paint(ctx, dots, dark, o.rMin, rgb);
};

// --- Rubik: bands twist in quarter turns, scramble → solve — solving --

export const drawRubik: ModeDraw = (ctx, size, t, dark, o, rgb) => {
  const cx = size / 2;
  const cy = size / 2;
  const R = (size / 2) * 0.82;
...
export const drawWave: ModeDraw = (ctx, size, t, dark, o, rgb) => {
  const cx = size / 2;
  const cy = size / 2;
