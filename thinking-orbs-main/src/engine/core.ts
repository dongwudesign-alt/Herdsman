...
/** Optional RGB ink tint. Defaults to grayscale when null. */
export type Rgb = [number, number, number];

/** Map a 0-255 ink level to an rgb fill/stroke string (or grayscale). */
function ink(level: number, alpha: number, rgb: Rgb | null): string {
  return rgb
    ? `rgba(${Math.round((rgb[0] * level) / 255)},${Math.round((rgb[1] * level) / 255)},${Math.round((rgb[2] * level) / 255)},${alpha})`
    : `rgba(${level},${level},${level},${alpha})`;
}

/**
 * Painter: z-sort far→near, matte grayscale dots (or a `rgb` tint scaled
 * by ink level). On dark substrates the ink value is mirrored (1 - white)
 * so near dots read bright — the same depth language on an inverted
 * substrate.
 */
...
export function paintLines(
  ...
  for (const l of lines) {
    ...
    const g = Math.round((dark ? 1 - w : w) * 255);
    ctx.strokeStyle = ink(g, alpha, rgb);
    ctx.lineWidth = l.w;
    ctx.beginPath();
